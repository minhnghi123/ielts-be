  ...
  private parseLines(lines: string[]): ParsedTest {
    ...
    while (i < lines.length) {
      ...
      if (/^---WRITING TASK (\d+)---/i.test(line)) {
        ...
        const taskNum = match
          ? parseInt(match[1], 10)
          : result.writingTasks.length + 1;
        i++;
        const { content, next } = this.collectUntil(
          lines,
          i,
          /^---END WRITING TASK/i,
        );
      ...
      if (/^---PASSAGE---/i.test(line)) {
        ...
        if (result.sections.length > 0) {
          result.sections[result.sections.length - 1].passage =
            content.join('\n');
        }
        i = next;
        continue;
      ...
      if (qType === 'matching_headings') {
        ...
        if (matchLine) {
          section.questions.push({
            ...
            config: { headings },
            answer: {
              correctAnswers: [matchLine[3].trim()],
              caseSensitive: false,
            },
          });
          i++;
